...
class DocumentDatabase:
    def __init__(self, reduce_memory=False, shelf_path=""):
        if reduce_memory:
            self.temp_dir = TemporaryDirectory()
            self.working_dir = Path(self.temp_dir.name)
            self.document_shelf_filepath = self.working_dir / 'shelf.db'
            #self.document_shelf = shelve.open('/cache/shelf.db', flag='n', protocol=-1)
            self.document_shelf = shelve.open(shelf_path, flag='n', protocol=-1)
            self.documents = None
        else:
            self.documents = []
...
def main():
    ...
    parser.add_argument('--shelf_path', type=str, default="", help="Where to put the shelf")
    ...
    doc_num = 0
    with DocumentDatabase(reduce_memory=args.reduce_memory, shelf_path=args.shelf_path) as docs:
